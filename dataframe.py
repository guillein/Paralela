...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def __plot_poly(self, ax, poligonos, color, **kwargs):
        ...
        if not isinstance(kwargs['color'], str) and color != False:
    ...
    def __plot_point(self, ax, puntos, color, **kwargs):
        ...
        # La añadimos a los ejes
        if not isinstance(kwargs['color'], str) and color != False:
            ax.scatter([p.x for p, _ in puntos], [p.y for p, _ in puntos], color = [c for _, c in puntos], **kwargs)2
        else:
    ...
    def __plot_linestring(self, ax, lineas, color, **kwargs):
        ...
        if not isinstance(kwargs['color'], str) and color != False:
            ...
            cjto = LineCollection(cjto, **kwargs)
    ...
    def plot(self, ax = False, color = False, **kwargs):
        ...
        if not isinstance(kwargs['color'], str) and color != False:
            ...
            objetos = self.select("geometry", "ColorPatin").rdd.map(lambda x : (x["geometry"].split('(')[0].upper().strip(), [(shapely.wkt.loads(x["geometry"]),  x['ColorPatin'])] ))
        else:
