...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def __plot_point(self, ax, puntos, color, **kwargs):
        ...
        elif not isinstance(color, str) and color != False:
    ...
    def plot_bar(self, x, ax = None, grosor = 0.35):
        ...
        nombres_cols = self.columns

        # Eliminamos la columna del eje x de los nombres
        try:
            nombres_cols.remove(x)
        except:
            raise IndexError("x debe ser el nombre de alguna columna del DF")

        # Cogemos el número de barras por valor de x
        num_barras = len(nombres_cols)

        # Pasamos a pandas para representar
        datos_repr = self.toPandas()

        # Creamos las posiciones del eje X
        etiquetas = list(range(len(datos_repr)))

        if num_barras > 2:
            raise Exception("Máximo dos barras por cada valor de x.")
        elif num_barras == 2:
            barra1 = ax.bar([float(e) - grosor/2 for e in etiquetas], list(datos_repr[nombres_cols[0]]), grosor, label = nombres_cols[0])
            barra2 = ax.bar([float(e) + grosor/2 for e in etiquetas], list(datos_repr[nombres_cols[1]]), grosor, label = nombres_cols[1])
        elif num_barras == 1:
            barra = ax.bar(etiquetas, datos_repr[nombres_cols[0]], grosor)
        else:
            raise("Debe haber alguna columna que no sea x para representar.")

        ax.legend()
        ax.set_xticks(etiquetas)
        ax.set_xticklabels(list(datos_repr[x]), rotation = 'vertical')
